...
func _init(chunk_arr_size, default_chunk_size):
	for chunk in range(0, chunk_arr_size):
		chunk_arr.append(DataFormats.Chunk.new(default_chunk_size))
...
func place_chunk(x: int, y: int, id: int, plane):
	...
	for block in range(0, chk.size.x*chk.size.y):
		...
		plane.get_handler(BlockHandler).place_block(block_x, block_y, chk.block_ids[block], plane, 

	...
	for chunk_ind in range(data.entries.size()):
		for y in range(data.tile_size.y):
			for x in range(data.tile_size.x):
				var blk : BaseChunkEntry.Block = data.entries[chunk_ind].blocks[x+y*data.tile_size.x]
				set_cell(0, Vector2(x+chunk_ind*data.tile_size.x,y),0,Vector2(blk.index,0),blk.flags&0b11)

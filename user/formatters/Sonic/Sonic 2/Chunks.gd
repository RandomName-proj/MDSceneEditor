...
func format(data: PackedByteArray, required : Dictionary, parameters : Dictionary) -> BaseFormat:
	...
	while chk_ind < data.size():
		var new_chunk := BaseChunkEntry.new(chunk_size,2)
		for blk_ind in range(0, chunk_size.x*chunk_size.y):
			...
			new_block.flags = block_data >> 2+8 & 0b11
			new_block.solids = [block_data >> 5+8 & 0b11, block_data >> 7+8 & 0b11]
